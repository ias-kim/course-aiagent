...
복잡한 작업을 한 번에 시키면 답변이 두루뭉술해지기 쉽습니다.
작업을 여러 단계로 나누고, 앞 단계의 결과를 다음 단계의 입력으로 연결합니다.
...
def call_llm(system, user_message):
    """반복되는 LLM 호출 코드를 줄이기 위한 작은 helper입니다."""
...
print("관찰 포인트: 체인은 앞 단계 출력을 다음 단계 입력으로 전달합니다.")
print("=" * 60)
...
# --- Prompt Chaining: 작업을 3단계로 나누어 연결 ---
print("\n--- Prompt Chaining (3단계) ---")

# Step 1: 긴 기사에서 핵심 사실만 먼저 뽑습니다.
...
# Step 2: 1단계 요약을 바탕으로 찬반 논점을 나눕니다.
...
# Step 3: 요약과 논점 분석을 합쳐 최종 평가를 만듭니다.
print("[Step 3] 종합 평가")
...
# 2부: 검증 체인 — 생성 후 검증
# ============================================================
# LLM이 만든 결과를 한 번 더 LLM에게 검토시키는 패턴입니다.
# Agent에서는 초안 생성 후 자기 검증(self-verification)에 자주 씁니다.
print()
print("=" * 60)
print("2부: 검증 체인 (생성 → 검증)")
print("관찰 포인트: LLM 검토 결과도 정답 보장은 아니며 테스트를 대신하지 않습니다.")
print("=" * 60)
...
# 3부: 변환 체인 — 단계적 변환
# ============================================================
# 하나의 텍스트를 번역, 키워드 추출, 요약처럼 단계별로 변환하는 패턴입니다.
print()
print("=" * 60)
