...
MCP(Model Context Protocol)란?
    AI 앱이 외부 도구와 데이터에 연결되는 방식을 표준화한 프로토콜입니다.
    Anthropic이 주도하는 오픈 표준으로, "AI의 USB-C"에 비유할 수 있습니다.

왜 MCP가 필요한가?
    Ch3에서 Tool Use를 직접 구현할 때 생기는 불편함:
    ...
    - 도구를 MCP 서버로 분리 → Agent 코드와 따로 관리할 수 있음
    - 표준화된 인터페이스 → 어떤 Agent든 같은 방식으로 연결 가능
    - 커뮤니티 MCP 서버 → 이미 만들어진 도구를 연결해 재사용 가능
...
# ============================================================
# Ch3와 Ch4의 차이를 코드 모양으로 비교합니다.
# ============================================================

# --- Ch3 방식: 도구 정의와 실행 로직을 Agent 안에 직접 넣는 방식 ---
ch3_example = """
# Agent 코드 안에 도구 명세와 실행 함수가 모두 들어감
tools = [
    {
        "name": "get_weather",
        "description": "날씨 조회",
...
# --- Ch4 방식: 도구를 MCP 서버로 분리하고 Agent는 연결만 담당 ---
ch4_example = """
...
# 아래 출력은 개념 비교를 눈으로 확인하기 위한 것입니다.
print("=" * 60)
