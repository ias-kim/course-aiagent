...
MCP 서버(schedule_server.py)가 제공하는 일정 도구를 사용하는 웹 기반 Agent입니다.

학습 목표:
    - MCP 서버의 도구를 Agent에 연결하는 방법을 이해한다
    - Agent 코드에는 DB 로직이 없고, 도구가 MCP 서버로 분리되어 있음을 확인한다
...
# Flask 요청 핸들러에서 MCP 세션과 도구 목록을 재사용하기 위한 상태입니다.
mcp_state = {
    "session": None,
    "tools": [],
    "loop": None,
...
async def init_mcp():
    """MCP 서버에 연결한 뒤, 서버가 제공하는 도구 목록을 Claude 형식으로 변환합니다."""
...
def start_mcp_in_background():
    """Flask와 별도로 asyncio 이벤트 루프를 돌려 MCP 세션을 유지합니다."""
    loop = asyncio.new_event_loop()
    mcp_state["loop"] = loop
    loop.run_until_complete(init_mcp())
    mcp_ready.set()  # Flask가 요청을 받기 전에 MCP 준비 완료를 알립니다.
    # MCP 세션이 살아 있으려면 이벤트 루프도 계속 살아 있어야 합니다.
    loop.run_forever()


# Flask가 요청을 받기 전에 MCP 서버 연결을 먼저 준비합니다.
...
def chat():
    """Agent 루프와 MCP 도구 호출 과정을 SSE로 스트리밍합니다."""
    data = request.json
            ...
            if response.stop_reason == "tool_use":
                ...
                for block in response.content:
                    ...
                    if block.type == "tool_use":
                        ...
                        # 실제 일정 DB 작업은 MCP 서버에 위임합니다.
                        # Flask는 동기 함수이므로 다른 스레드의 asyncio 루프에 코루틴을 제출합니다.
...
#
# 1. GET /
#    채팅 UI와 사용 가능한 일정 도구 목록을 보여주는 메인 페이지
#
# ─────────────────────────────────────────────────────────────
#
# 2. POST /chat
#    사용자 메시지를 받아 MCP Agent 루프를 실행하고 SSE로 결과를 보냅니다.
#
#    Request:
